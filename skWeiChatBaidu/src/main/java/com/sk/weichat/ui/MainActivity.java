...
import com.fanjun.keeplive.receiver.ExampleUtil;
import com.fanjun.keeplive.receiver.LocalBroadcastManager;
import com.google.android.gms.common.ConnectionResult;
import com.google.android.gms.common.GoogleApiAvailability;
import com.redchamber.auth.AuthenticationRealActivity;
...
import com.yujianni.app.BuildConfig;
...
public class MainActivity extends BaseActivity implements PermissionUtil.OnRequestPermissionsResultCallbacks {
    ...
    public static void requestQrCodeScan(Activity ctx) {
        int size = ScreenUtil.getScreenWidth(getContext()) / 16 * 9;
        // 生成底部自己的二维码bitmap
...
//        showDeviceLock();
    ...
    protected void onRestart() {
        ...
        // 重新定位并更新位置信息
        if ( MyApplication.getInstance().mActivityCount == 0) {
            MyApplication.getInstance().getBdLocationHelper().isLocationUpdate = false;
    ...
    public void getNearIndexUser(String onlineFirst, String cityName, String provinceName) {
...
//        Toast.makeText(mContext,String.valueOf(longitude),Toast.LENGTH_SHORT).show();
...
//                            mAdapter.setNewData(result.getData());
                            EventBus.getDefault().postSticky(new UpdateCityOnlineEvent("", "", "0"));
                            MsgBroadcast.broadcastMsgUiUpdate(mContext);
                        } else {
    ...
    // 从极光推送接收消息 for receive customer msg from jpush server
...
//        getSupportActionBar().hide();
    ...
    private void initDatas() {
        ...
        sp = this.getSharedPreferences("mypd", Context.MODE_PRIVATE);
        editor = sp.edit();
        Boolean isClick = sp.getBoolean("isClick",false);
        if (!isClick){
            showTipOverlay(this);
        }
   ...
   // 请求悬浮通知权限
   private  void showTipOverlay (Activity activity) {

       SelectionFrame dialog = new SelectionFrame(this);
       dialog.setSomething("请开启权限，方便消息通知",
               "点击通知管理→开启横幅(悬浮通知)和锁屏通知", new SelectionFrame.OnSelectionFrameClickListener() {
                   @Override
                   public void cancelClick() {
                   }
                   @Override
                   public void confirmClick() {
                       PermissionUtil.requestOverlayPermission(activity);
...
//            VivoPushMessageReceiver.init(this);
    ...
    public void loginOut() {
        ...
        if (MyApplication.getInstance().mUserStatus == LoginHelper.STATUS_USER_TOKEN_OVERDUE) {
            UserCheckedActivity.start(getContext());
        }
        finish();
    }
...
*/
    ...
    */
    ...
    private void sendClientid(){
        ...
        if(!sp.getString("clientid","").equals("")) {

            String cid = sp.getString("clientid","");
            HashMap<String, String> params = new HashMap<String, String>();
//            params.put("access_token", coreManager.getSelfStatus().accessToken);
            params.put("userId", coreManager.getSelf().getUserId());
            params.put("cid",cid);
            HttpUtils.post().url(coreManager.getInstance(this).getConfig().GETUI_CID)
                    .params(params)
                    .build()
                    .execute(new BaseCallback<Void>(void.class) {
                        @Override
                        public void onResponse(ObjectResult<Void> result) {
                            Log.e("clientid", String.valueOf(result.getResultCode()));
                            if (result.getResultCode() == 1) {
                                Log.e("clientid", "clientid已发送");

                            } else {
                                Log.e("clientid", "clientid未发送");
                            }
