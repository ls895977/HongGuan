...
import com.redchamber.login.NewAAActivity;
...
public class ImprovePersonalInfoActivity extends BaseLoginActivity implements View.OnClickListener {
    ...
    protected void initView() {
        ...
        // 上传头像提示
        upHeadImgTip = findViewById(R.id.upHeadImgTip);
        ...
        if (getIntent() != null) {
            ...
            if (mRegisterInfoBean != null) {
                isWomen = mRegisterInfoBean.sex.equals("0");
                if (isWomen){
                    upHeadImgTip.setText("请上传带本人面容头像，否则可能会导致无法通过认证！");
                }
            }else {
                userLevels = UserLevelUtils.getLevels(MyApplication.mMyHomepageBean.userLevel);
                isWomen2 = userLevels[0];
                if (isWomen2){
                    upHeadImgTip.setText("请上传带本人面容头像，否则可能会导致无法通过认证！");
                }
            }
        ...
        switch (view.getId()) {
            ...
            case R.id.btn_next:
                ...
                if (TYPE_EDIT_INFO == mPageType) {
                    ...
                    NewAAActivity.start(this, mRegisterInfoBean);
                }
                break;
        }
    }
