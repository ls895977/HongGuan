...
public class MineFemaleFragment extends BaseFragment {
    ...
    @BindView(R.id.restore_img)
    RelativeLayout restoreImg;
    ...
    protected void initView() {
        ...
        if (!userLevels[0]) {//男性不展示恢复销毁照片功能
            restoreImg.setVisibility(View.GONE);
        }
    }
    ...
    @OnClick({R.id.tv_my_comment, R.id.iv_avatar, R.id.ll_person_info, R.id.ll_pocket, R.id.ll_my_like, R.id.rl_auth, R.id.rl_vip,
            ...
            R.id.rl_photo_privacy, R.id.rl_black_list, R.id.rl_setting, R.id.rl_share, R.id.rl_email,R.id.restore_img})
...
//                shareDialog.show();
                ...
                textIntent.putExtra(Intent.EXTRA_TEXT, "http://www.ees100.com/");
                getContext().startActivity(Intent.createChooser(textIntent, "分享"));
                break;
            case R.id.rl_email:
                ClipboardManager cm = (ClipboardManager) getContext().getSystemService(Context.CLIPBOARD_SERVICE);
                ClipData mClipData = ClipData.newPlainText("Label", "微信公众号：遇见小助手");
                cm.setPrimaryClip(mClipData);
                ToastUtils.showToast("已复制到剪切板");
                break;
            case R.id.restore_img:
                restoreImg();
                break;
        }
    }
...
//            params.put("userId", coreManager.getSelf().getUserId());
            ...
            HttpUtils.post().url(coreManager.getInstance(getContext()).getConfig().RESTORE_IMG)
                    .params(params)
                    .build()
    ...
    private void updateUserInfo(MyHomepageBean homepageBean) {
        ...
        if (homepageBean.photoAlbum != null && homepageBean.photoAlbum.photos != null && homepageBean.photoAlbum.photos.size() > 0) {
            ...
            if (1 == homepageBean.photoAlbum.type) {
                ...
                mTvPayUnlock.setText(String.format("付费解锁(%d)糖果", homepageBean.photoAlbum.coin));
            } else {
