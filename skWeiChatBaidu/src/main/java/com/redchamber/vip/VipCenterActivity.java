...
public class VipCenterActivity extends BaseActivity {
    ...
    private void initWebView(String userId) {
        if (TextUtils.isEmpty(sex)) {
            if (MyApplication.mMyHomepageBean != null && !TextUtils.isEmpty(MyApplication.mMyHomepageBean.userLevel)) {
                char[] arr = MyApplication.mMyHomepageBean.userLevel.toCharArray();
                sex = String.valueOf(arr[0]);
            } else {
                int mode = (int) SharedPreferencesUtil.getData("sex", 0);
                if (mode == 1) {
                    sex = "1";
                } else {
                    sex = "0";
        ...
        if (sex != null && sex.equals("1")) {
            URL = String.format("http://api.bao.0575yw.com/pages/hgH5pay/index.html#/privilege_man?userId=%s&sex=%s", userId, "1");
        } else {
            URL = String.format("http://api.bao.0575yw.com/pages/hgH5pay/index.html#/privilege_woman?userId=%s&sex=%s", userId, "0");
        }
