...
import com.sk.weichat.util.DistanceUtils;
...
//        }
//        Log.i("aaa", "金纬度==" + latitude + "," + longitude);
//        if (latitude != 0 && longitude != 0 && homepageBean.lat != 0 && homepageBean.lng != 0) {
//            LatLng point_start = new LatLng(homepageBean.lat, homepageBean.lng);
//            LatLng point_end = new LatLng(latitude, longitude);
...
//            }
//            mTvDistance.setText(value);
//        } else {
//            mTvDistance.setText(R.string.this_friend_not_open_position);
//        }
        if (latitude != 0 && longitude != 0 && homepageBean.lat != 0 && homepageBean.lng != 0) {
            if (homepageBean.lat < 0) {
                mTvDistance.setText("保密");
            } else {
                float distance = (float) DistanceUtils.getDistance(longitude, latitude, homepageBean.lng, homepageBean.lat);
                int dis = Math.round(distance);
                String value = String.format("%skm", dis);
                mTvDistance.setText(value);
            }
        } else {
            mTvDistance.setText("未知");
        }
        mTvCity.setText(homepageBean.registerCity);
        mTvOnline.setText(homepageBean.onlinestate);
    ...
    private void showBarCom(String title, int resultCode, String type) {
        ...
        if (resultCode == REAL_PEOPLE_VERIFY_FIRST) {
            ...
            center = type.equals(EVEN_WHEAT) ? "连麦（" + mUserHomePageBean.lianMaiGold + "糖果）" : "私聊（" + mUserHomePageBean.lianMaiGold + "糖果";
            ...
            if (MyApplication.mMyHomepageBean != null) {
                ...
                if (userLevels[1]) {//VIP
                    if (type.equals("005")) {
                        ...
                        center = type.equals(EVEN_WHEAT) ? "连麦（" + mUserHomePageBean.lianMaiGold + "糖果）" : "私聊（" + mUserHomePageBean.privateChatGold + "糖果)";
    ...
    private void VipOrPay() {
        ...
        EvenWheatCodeDialog commCodeDialog = new EvenWheatCodeDialog(FriendHomePageActivity.this, "是否与TA私聊？", "成为会员，免费私聊", "私聊（" + mUserHomePageBean.privateChatGold + "糖果)");
        commCodeDialog.show();
        commCodeDialog.setOnConfirmListener(new EvenWheatCodeDialog.OnConfirmListener() {
            @Override
            public void onConfirmClick() {
    ...
    private void VipEvenOrPay() {
        EvenWheatCodeDialog commCodeDialog = new EvenWheatCodeDialog(FriendHomePageActivity.this, "是否与TA连麦？", "成为会员，免费连麦", "连麦（" + mUserHomePageBean.lianMaiGold + "糖果)");
        commCodeDialog.show();
        commCodeDialog.setOnConfirmListener(new EvenWheatCodeDialog.OnConfirmListener() {
            @Override
            public void onConfirmClick() {
    ...
    private void payRed(String resultCode) {
        ...
        CommCodeDialog commCodeDialog = new CommCodeDialog(FriendHomePageActivity.this, "次数已用完", "支付" + price + "糖果");
        commCodeDialog.show();
    ...
    private void setUserLevel(String userLevel) {
        ...
        if (userLevels[2] && userLevels[0]) {//女神
...
//                    album(data.getStringArrayListExtra(PhotoPickerActivity.EXTRA_RESULT), isOriginal);
    ...
    private void updateAlbumStatus(UserHomePageBean.UserAlbumVo userAlbumVo) {
        if (userAlbumVo != null) {
            ...
            if (1 == mAlbumType && 0 == lockFlag) {
                ...
                mTvLock.setText("解锁她的相册(" + userAlbumVo.coin + "糖果)会员免费");
            } else if (2 == lockFlag) {
                mLlPermission.setVisibility(View.VISIBLE);
    ...
    private void nonVipUnlockAlbum() {
        CommonHintDoubleDialog doubleDialog = new CommonHintDoubleDialog(FriendHomePageActivity.this,
                ...
                "付费解锁(" + mUserHomePageBean.userAlbumVo.coin + "糖果)");
        doubleDialog.show();
